...
ke.login(token="El68IkFsh4QmEq2bblj0.Y00CqhaENcteJzCWN27b8a.aCmhSEu12H2pZHt1muDK7DmJTRIIUUBhrnywL7V+8aI=")
ke.loginResult()

ku = LINETCR.LINE() #Orang Gila
...
def bot(op):
    try:
        ...
        if op.type == 17: #awal 17 ubah 13
           if wait["Protectjoin"] == True:
               if op.param3 not in admin : # Awalnya admin doang
                   random.choice(KAC).kickoutFromGroup(op.param1,[op.param2])
        #------Joined User Kick start------#
        
        if op.type == 19:
           if op.param3 in Bots: #AWAL DEF BUKAN KAC dan Bots
              random.choice(KAC).kickoutFromGroup(op.param1,[op.param2])
        ...
        if op.type == 19:
           if op.param3 in admin: #awal nya admin aja
              random.choice(KAC).kickoutFromGroup(op.param1,[op.param2])
    ...
    #------------SC Like-------------------
              ...
              if msg.from_ in owner:
                ...
                cl.sendText(msg.to,"Bot Siap Like Status Owner")
                try:
                  likePost()
                except:
                    pass
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
            elif msg.text in ["Absen","Absen bot","Absen dulu","Absen dong"]:
              if msg.from_ in admin:
                cl.sendText(msg.to,"Luffy Pertamax")
                ki.sendText(msg.to,"Zorro Kedua")
                kk.sendText(msg.to,"Sanji Ketigax")
                kc.sendText(msg.to,"Ussop On")
                ks.sendText(msg.to,"ZzzzZ")
                ka.sendText(msg.to,"........")
                kb.sendText(msg.to,"Franky On")
                ko.sendText(msg.to,"On Boss")
                ke.sendText(msg.to,"Nami On Sayang")
                ku.sendText(msg.to,"Robin Siap")
